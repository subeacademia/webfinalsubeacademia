...
function analizarContextoYGenerarObjetivos(context: string): any[] {
  ...
  if (context.toLowerCase().includes('pequeña') || context.toLowerCase().includes('startup')) {
    ...
    return objetivosBase.filter(obj => 
      obj.prioridad === 'alta' && 
      (obj.categoria === 'Capacitación' || obj.categoria === 'Procesos')
    ).slice(0, 4);
    ...
    return objetivosBase.filter(obj => 
      obj.prioridad === 'alta' || obj.prioridad === 'media'
    ).slice(0, 6);
    ...
    return objetivosBase.filter(obj => 
      obj.categoria === 'Innovación' || obj.categoria === 'Analítica' || obj.categoria === 'CX'
    ).slice(0, 5);
    ...
    return objetivosBase.filter(obj => 
      obj.categoria === 'CX' || obj.categoria === 'Procesos' || obj.categoria === 'Capacitación'
    ).slice(0, 5);
  }
