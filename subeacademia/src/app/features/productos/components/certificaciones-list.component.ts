      ...
      <div class="grid md:grid-cols-12 gap-8">
        <aside class="md:col-span-3">
          <form [formGroup]="filtersForm" class="sticky top-6 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg p-4 space-y-4">
            <input type="text" formControlName="search" [placeholder]="'productos.lists.certificaciones.filters.search_placeholder' | i18nTranslate" class="w-full px-4 py-2 rounded-md bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700" />
            <select formControlName="nivel" class="w-full px-4 py-2 rounded-md bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700">
              <option value="">{{ 'productos.lists.certificaciones.filters.all_levels' | i18nTranslate }}</option>
            ...
            <select formControlName="precio" class="w-full px-4 py-2 rounded-md bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700">
              <option value="">{{ 'productos.lists.certificaciones.filters.any_price' | i18nTranslate }}</option>
              <option value="lt200">{{ 'productos.lists.certificaciones.filters.lt200' | i18nTranslate }}</option>
              <option value="200-600">{{ 'productos.lists.certificaciones.filters.200-600' | i18nTranslate }}</option>
              <option value="gt600">{{ 'productos.lists.certificaciones.filters.gt600' | i18nTranslate }}</option>
            </select>
          </form>
        </aside>
              ...
              <div class="p-6">
                ...
                <div class="space-y-2 mb-4">
                  <div class="flex items-center text-sm text-gray-500 dark:text-gray-400">
                    <span class="font-medium mr-2">{{ 'productos.lists.certificaciones.labels.entity' | i18nTranslate }}</span>
                    {{ certificacion.entidadCertificadora }}
                  </div>
                  <div class="flex items-center text-sm text-gray-500 dark:text-gray-400">
                    <span class="font-medium mr-2">{{ 'productos.lists.certificaciones.labels.level' | i18nTranslate }}</span>
                ...
                <div class="flex items-center justify-between">
                  ...
                  <a [routerLink]="['/productos', certificacion.slug]" class="bg-purple-600 hover:bg-purple-700 text-white px-4 py-2 rounded-lg transition-colors">
                    {{ 'productos.lists.certificaciones.cta_view_details' | i18nTranslate }}
                  </a>
                </div>
              </div>
            </div>
